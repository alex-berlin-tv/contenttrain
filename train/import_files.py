...
def handle_item(episodes: NocoEpisodes, episode: NocoEpisode, count: int, total: int) -> int:
    ...
    if episode.source_state != SourceState.DISA_SERVER or \
       episode.file_on_edit_state == FileState.DONE or \
       episode.is_transcoded:
        return count
    ...
    if episode.file_on_edit_state == FileState.EXISTS:
        ...
        return count + 1
    print(f"{progress} Copy from server {description}")
